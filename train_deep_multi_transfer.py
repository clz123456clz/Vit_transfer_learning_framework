...
from model.deepvit_transfer import ViT
...
def build_transfer_optimizer(
    ...
    if head_params:
        # head does not need weight decay
        param_groups.append({
            "params": head_params,
            "lr": lr_head,
            "weight_decay": 0.0,
...
def train(
    ...
    axes = make_training_plot("AdamWvit_data_aug_label_smoothing_chosen_mixup_transfer Training")
    ...
    while curr_patience < patience:
        # Train model
        train_epoch(tr_loader, model, criterion, optimizer, device=device, mixup_alpha=0.2)
    ...
    plt.savefig(f"adamWvit_transfer_image_aug_label_smoothing_mixup_mlp_only.png", dpi=200)
    plt.ioff()
    plt.show()
...
def main() -> None:
    ...
    freeze_none = ViT(16, 8, 1024, 16, device=device, datatype=datatype, num_classes=2).to(device)
    ...
    train(tr_loader, va_loader, te_loader, freeze_none, "adamWvit_transfer_image_aug_label_smoothing_chosen_mixup", device)
