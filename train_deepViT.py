...
def main():
    ...
    model = ViT(16, 8, 1024, 16, device=device, datatype=datatype)
    ...
    lr=config("adamWvit.learning_rate"), betas=(0.9, 0.999), weight_decay=0.05
)
    ...
    print("Loading ViT...")
    model, start_epoch, stats = restore_checkpoint(model, config("adamWvit.checkpoint"))
    model = model.to(device)
    print("model on:", next(model.parameters()).device) 
    ...
    axes = make_training_plot(name="adamWViT Training")
