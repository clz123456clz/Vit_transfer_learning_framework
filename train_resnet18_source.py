...
def main():
    ...
    tr_loader, va_loader, te_loader, _ = get_train_val_test_loaders(
        task="source",
        batch_size=config("resnet18_source.batch_size"),
    )
    datatype = None
    ...
    lr=config("resnet18_source.learning_rate"), betas=(0.9, 0.999), weight_decay=0.05
)
    ...
    model, start_epoch, stats = restore_checkpoint(model, config("resnet18_source.checkpoint"))
    model = model.to(device)
    print("model on:", next(model.parameters()).device) 
    ...
    axes = make_training_plot(name="resnet18_source Training")
    ...
    while curr_patience < patience:
        ...
        save_checkpoint(model, epoch + 1, config("resnet18_source.checkpoint"), stats)
    ...
    plt.savefig(f"resnet18_source_training_plot_patience={patience}.png", dpi=200)
    plt.ioff()
    plt.show()
